...
// Check if Network.Testnet is available, otherwise use the fallback
let sdk;
if (Network && Network.Testnet) {
  sdk = new SDK(Network.Testnet);
} else {
  console.warn("Network.Testnet is not defined, using fallback");
  sdk = new SDK("https://testnet.aptoslabs.com");
}
...
function App() {
  ...
  const incoming = async () => {
    const res = await sdk.stream.getIncomingStreams(
      "0x20f0cbe21cb340fe56500e0889cad03f8a9e54a33e3c4acfc24ce2bdfabc4180"
    );
  ...
  const outgoing = async () => {
    const res = await sdk.stream.getOutgoingStreams(
      "0x20f0cbe21cb340fe56500e0889cad03f8a9e54a33e3c4acfc24ce2bdfabc4180"
    );
