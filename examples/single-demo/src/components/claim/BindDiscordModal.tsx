...
const checkAddressBind = async (address: string) => {
  try {
    const res = await axios.get(`https://bind.moveflow.org/checkAddressBind?address=${address}`, {
      timeout: 5000,
    })
    // console.log(res)
    if (res.status === 200) {
          ...
          <AutoColumn gap="md" style={{ padding: '1rem', paddingTop: '0', paddingBottom: '2rem' }} justify="center">
            <ButtonPrimary
              ...
              onClick={() => {
                ...
                window.open(
                  `https://discord.com/oauth2/authorize?response_type=code&client_id=1035092636085796874&scope=identify%20guilds%20guilds.members.read&state=${account}&redirect_uri=https://bind.moveflow.org/bind&prompt=consent`,
                  '_blank'
                )
                // window.open(
