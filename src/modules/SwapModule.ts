...
export class SwapModule implements IModule {
  ...
  swapPayload({
    ...
    const struct = fixedCoin === 'from' ? 'swap_exact_coins_for_coins_entry' : 'swap_coins_for_exact_coins_entry'
    
    const functionName = composeType(modules.Scripts, struct)
  ...
  /**
   * The function will return all pairs created in moveflow, with CoinX and CoinY full name
   * @returns all pairs
   */
  ...
  /**
   * The function will return all pairs created in moveflow, with coin full name and reserve meta
   * @returns 
   */
  ...
  async getLPCoinAPYBatch(deltaVersion?: Decimal | string): Promise<LPCoinAPYBatchReturn> {
    ...
    const queryLedgerVersion = d(currentLedgerVersion).sub(queryDeltaVersion).gte(d(oldestLedgerVersion))
        ? d(currentLedgerVersion).sub(queryDeltaVersion)
        : d(oldestLedgerVersion)
